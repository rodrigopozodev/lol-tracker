...
export default function DashboardNav({ authedInitial }: { authedInitial?: boolean }) {
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  const [authed, setAuthed] = useState<boolean | null>(
    typeof authedInitial === "boolean" ? authedInitial : null
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (mounted) setAuthed(!!json?.authenticated);
      } catch {
  ...
  return (
    <nav className="px-4 py-3 flex items-center justify-center gap-6">
      ...
      {/* Botón Login/Logout según estado de sesión */}
      {authed ? (
        <Button
          onClick={logout}
          disabled={loading}
        ...
        >
          <svg
            ...
            strokeWidth={2}
          >
            {/* Icono de usuario para indicar login */}
